...
public class Test {
	...
	public static Actions clickOutsideElement(String elementXpath) {
		Actions action = new Actions(pageBrowser.driver);
		;
		WebElement theElement = waitElement(elementXpath);
		try {
			action.moveToElement(theElement, -1, 0).click().build().perform();
		} catch (Exception e) {
			Assert.assertFalse("clickOutsideElement failed: " + e.getMessage(), true);
		}
		return action;
	}
